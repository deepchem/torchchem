...
#     self.fc_layer_dims = [self.conv_layer_dims[self.n_conv_layers]
#                          ] + self.fc_layer_dims
...
#     self.input_x = torch.FloatTensor(-1, self.net.max_n_atoms,
#                                      self.net.n_atom_types)
#     self.input_g = torch.FloatTensor(-1, self.net.max_n_atoms,
#                                      self.net.max_n_atoms)
...
#     self.input_x, self.input_g, self.label = self.input_x.cuda(
#     ), self.input_g.cuda(), self.label.cuda()
...
#       batch_inds = [
#           train_inds[idx] for idx in range(b * batch_size, (b + 1) * batch_size)
#       ]

#       train_x_batch = np.concatenate(
#           [np.expand_dims(train_features[idx]["x"], 0) for idx in batch_inds],
#           axis=0)
#       train_g_batch = np.concatenate(
#           [np.expand_dims(train_features[idx]["g"], 0) for idx in batch_inds],
#           axis=0)
...
#         o.append(
#             transformer.inverse_transform(output.data.cpu().numpy().reshape(
#                 (-1, 1))).flatten())
#         l.append(
#             transformer.inverse_transform(y_train[batch_inds].reshape(
#                 (-1, 1))).flatten())
#       else:
...
#       batch_inds = [
#           test_inds[idx] for idx in range(b * batch_size, (b + 1) * batch_size)
#       ]

#       test_x_batch = np.concatenate(
#           [np.expand_dims(test_features[idx]["x"], 0) for idx in batch_inds],
#           axis=0)
#       test_g_batch = np.concatenate(
#           [np.expand_dims(test_features[idx]["g"], 0) for idx in batch_inds],
#           axis=0)
...
#                 (-1, 1))).flatten())
#         l.append(
#             transformer.inverse_transform(y_test[batch_inds].reshape(
#                 (-1, 1))).flatten())
#       else:
#         o.append(output.data.cpu().numpy().reshape((-1, 1)).flatten())
...
#       task_loss = -(task_label * torch.log(scores)) + (
#           1. - task_label) * torch.log(1. - task_label)
...
class WeightedBCEWithLogits(nn.Module):
  ...
  def forward(self, output, y, w):
    output = output.float()
    y = y.float()
    err = self.loss_fn(output, y)
    err = (err * w).sum()
    return err
...
class GraphConvolutionNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.n_node_feautres = n_node_features
    self.n_tasks = n_tasks
    
    self.n_conv_layers = n_conv_layers
    assert len(conv_layer_dims) == self.n_conv_layers
    self.n_fc_layers = n_fc_layers
    assert len(fc_layer_dims) == self.n_fc_layers    
    self.dropout = dropout

    self.conv_ops = nn.ModuleList()
    n_in = n_node_features
    for layer_idx in range(self.n_conv_layers):
      n_out = conv_layer_dims[layer_idx]
      self.conv_ops.append(GCNConv(n_in, n_out))
      n_in = n_out

    self.fc_ops = nn.ModuleList()    
    for layer_idx in range(self.n_fc_layers):
      n_out = fc_layer_dims[layer_idx]
      op = nn.Sequential(
    ...
    self.task_fc = nn.Linear(n_in, n_tasks)
    self.post_op = post_op

  def forward(self, data):
    x, edge_index = data.x, data.edge_index
    if not 'batch' in data.__dict__:
      batch = t.zeros((x.shape[0],)).long()
    else:
      batch = data.batch
      
    for conv_op in self.conv_ops:
      x = conv_op(x, edge_index)
      x = F.relu(x)
      x = F.dropout(x, training=self.training)

    x = global_max_pool(x, batch)
    for fc_op in self.fc_ops:
      x = fc_op(x)
    
    output = self.task_fc(x)
    return output
  
  def predict(self, data):
    output = self.forward(data)
    if not self.post_op is None:
      output = self.post_op(output)
    return output
  
  
class GraphConvolutionModel(object):
  ...
  def train_epoch(self,
                  ...
                  batch_size=32,
                  shuffle=True):
    loader = tg.data.DataLoader(dataset, 
    ...
    for batch in loader:
      ...
      y = batch.y.reshape((-1, self.net.n_tasks))
      w = batch.w.reshape((-1, self.net.n_tasks))
      err = self.criterion(output, y, w)
      err.backward()
      total_loss += err.item()
      self.optimizer.step()
    print("Loss of the epoch: %f" % (total_loss/len(dataset)))
